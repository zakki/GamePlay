...
public:
    ...
    /**
     * Visits each node in the scene and calls the specified functor.
     *
     * Calling this method invokes the specified functor for each node
     * in the scene hierarchy.
     *
     * The visitFunc parameter must be a pointer to a method that has a bool
     * return type and accepts a single parameter of type Node*.
     *
     * A depth-first traversal of the scene continues while the visit method
     * returns true. Returning false will stop traversing further children for
     ...
     *
     * @param instance The pointer to an instance of the object that contains visitMethod.
     * @param visitFunc The pointer to the class method to call for each node in the scene.
     */
    template <class T>
